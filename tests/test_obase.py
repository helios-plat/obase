...
# Ensure obase is importable from the source tree
...
class TestLogging:
    def test_setup_logging_idempotent(self):
        ...
        olog.setup_logging("INFO")  # second call must not raise
...
class TestConfig:
    ...
    def test_env_var_fallback_when_not_in_cfg(self, monkeypatch):
        ...
        cfg.load_config(Path("/nonexistent/path/config.yaml"))
        # get() falls back to os.environ
        assert cfg.get("SOME_DIRECT_ENV") == "direct_value"


# ---------------------------------------------------------------------------
# cost_tracker_stratum
# ---------------------------------------------------------------------------
...
class TestBootstrapStratum:
    def test_bootstrap_runs_without_error(self, tmp_path, monkeypatch):
        """bootstrap() should create all required stratum directories."""
        # Redirect home to tmp_path so we don't touch real ~/.stratum
